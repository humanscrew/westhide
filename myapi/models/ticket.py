...
class TicketLaiu8(db.Model):
    ...
    channel_name = db.Column(db.String(80), index=True, comment="渠道=来游吧/北海窗口/涠洲窗口/票务室/内部订票/驻岛订票...")
    ...
    ticket_status = db.Column(db.String(80), index=True, comment="船票状态=已取消/出票成功/一检/二检/改签废票/已退款/已改签/已升舱...")
    departure_datetime = db.Column(db.DATETIME, index=True, comment="航班时间")
    line_name = db.Column(db.String(80), index=True, comment="航线名称=北海-涠洲/涠洲-北海/北海-海口...")
    ...
    full_ticket_price = db.Column(db.Float, index=True, comment="原价")
    discount_price = db.Column(db.Float, index=True, comment="票价折扣")
    ticket_price = db.Column(db.Float, index=True, comment="实际票价=原价-票价折扣")
    get_voucher = db.Column(db.Float, index=True, comment="产生的积分")
    use_voucher = db.Column(db.Float, index=True, comment="使用的积分")
    ...
    real_price = db.Column(db.Float, index=True, comment="实际支付价格=实际票价-使用的积分=<微信订单金额>/<支付宝收入金额>")
...
class TicketLaiu8Refund(db.Model):
    ...
    fee = db.Column(db.Float, index=True, comment="手续费")
    refund_amount = db.Column(db.Float, index=True, comment="退款金额")
...
class Ticket2Finance(db.Model):
    ...
    ticket_price = db.Column(db.Float, comment="实际票价=原价-票价折扣")
    get_voucher = db.Column(db.Float, comment="产生的积分")
    use_voucher = db.Column(db.Float, comment="使用的积分")
