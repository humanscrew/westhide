from myapi.extensions import db, cdb
...
class TicketLaiu8(db.Model):
    ...
    create_time = db.Column(db.TIMESTAMP, index=True, comment="下单时间")
    ...
    departure_datetime = db.Column(db.TIMESTAMP, index=True, comment="航班时间")
    line_name = db.Column(
    ...
    payment_time = db.Column(db.TIMESTAMP, index=True, comment="支付时间")
    ...
    change_time = db.Column(db.TIMESTAMP, index=True, comment="操作时间")
...
class TicketLaiu8Refund(db.Model):
    ...
    ticket_refund_finish_time = db.Column(db.TIMESTAMP, index=True, comment="实际退款时间")
    refund_id = db.Column(
...
class Laiu8Client(db.Model):
    ...
    cooperate_start_time = db.Column(db.TIMESTAMP)
    cooperate_end_time = db.Column(db.TIMESTAMP)
    manager = db.Column(db.String(80))
    sales = db.Column(db.DECIMAL(10, 3))


class Ticket2Finance(db.Model):
    __tablename__ = "ticket_2_finance"
    id = db.Column(db.Integer, primary_key=True)
    type = db.Column(db.String(20))
