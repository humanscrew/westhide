...
class Kafka:
    def __init__(
        ...
        self.topic = topic
        self.key_serializer = (
            key_serializer if key_serializer else lambda k: json.dumps(k).encode()
        )
        self.value_serializer = (
            value_serializer if value_serializer else lambda v: json.dumps(v).encode()
        )
    ...
    def get_consumer(self, topic=None, group_id=None):
        ...
        if topic and group_id:
            return KafkaConsumer(
                topic, bootstrap_servers=self.bootstrap_servers, group_id=group_id
            )
        else:
            return None
    ...
    def consumer(self, topic=None, group_id=None):
        consumer = (
            self.get_consumer(topic, group_id) if topic and group_id else self.consumer
        )
